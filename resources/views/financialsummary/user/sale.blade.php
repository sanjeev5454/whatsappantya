...
<form action="{{ url('financialsummary/dataSubmitSales') }}" name="financial_summery" id="financial_summery" method="post">
@csrf
...
$(function(){
   ...
   var id_db = $('#h-'+id_rel).val();
     $.ajaxSetup({
      ...
      $.ajax({
        url: "{{ url('financialsummary/dataSubmitSales') }}" ,
        type: "POST",
...
$(function(){
     ...
     $.ajaxSetup({
      ...
      $.ajax({
        type: "GET",
		url: "{{ url('financialsummary/dataRemoveSales')}}/"+id,
		data: {id:id},
        success: function( response ) {
