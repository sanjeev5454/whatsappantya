...
<form action="{{ url('financialsummary/dataSubmitExpensesX') }}" name="financial_summery" id="financial_summery" method="post">
@csrf
  <div class="table-scrollable">
      ...
      <tbody>
        ...
        <tr class="variable_expenses" id="remove-{{$i}}">
		  ...
		  <select cus="a-{{$i}}"  class="a-{{$i}} e-{{$i}} attribute" autocomplete="off" name="row[{{ $i }}][name]" style="display:none;">
		  @foreach($variable_expenses_select as $expenses_dropdown)
		  <option @if($variable_expenses['name']==$expenses_dropdown['name']) selected="selected" @endif value="{{ $expenses_dropdown['name'] }}">{{ $expenses_dropdown['name'] }}</option>
		  @endforeach
		  </select>
        ...
        <tr class="fixed_expenses" id="remove-{{$i}}">
		  ...
		  <option @if($fixed_expenses['name']==$fixed_expenses_dropdown['name']) selected="selected" @endif value="{{ $fixed_expenses_dropdown['name'] }}">{{ $fixed_expenses_dropdown['name'] }}</option>
		  @endforeach
		  </select>
...
$(function(){
   ...
   var type = $('#type-'+id_rel).val();
     $.ajaxSetup({
      ...
      $.ajax({
        url: "{{ url('financialsummary/dataSubmitExpensesX') }}" ,
        type: "POST",
        ...
        success: function( response ) {
		  ...
		  $('.exp-amt-'+id).attr('id', data['variable_expenses_id']);
		  }
		  
		  if(type=='fixed_expenses'){
		  $('#h-'+id_rel).val(data['fixed_expenses_id']);
		  $('.exp-amt-'+id).attr('id', data['fixed_expenses_id']);
		  }
		  
		  if(type=='average_monthly'){
		  $('#h-'+id_rel).val(data['average_monthly_id']);
		  $('.exp-amt-'+id).attr('id', data['average_monthly_id']);
		  }
...
$(document).on('click','.add-expenses',function(){
     $.ajaxSetup({
	  ...
	  var type = 'expensesx';
      $.ajax({
        url: "{{ url('financialsummary/ajaxExpensesX') }}" ,
        type: "POST",
        data: {rowCount:rowCount,type:type},
        success: function( response ) {
          //alert(response);
		  if(response!='')
		  {
...
$(document).on('click','.add-fixed-expenses',function(){
     $.ajaxSetup({
	  ...
	  var type = 'expensesx';
      $.ajax({
        url: "{{ url('financialsummary/ajaxFixedExpensesX') }}" ,
        type: "POST",
        data: {rowCount:rowCount,type:type},
        success: function( response ) {
          //alert(response);
...
$(document).on('click','.add_average_monthly',function(){
     $.ajaxSetup({
	  ...
	  var type = 'expensesx';
      $.ajax({
        url: "{{ url('financialsummary/ajaxAverageExpensesX') }}" ,
        type: "POST",
        data: {rowCount:rowCount,type:type},
        success: function( response ) {
          //alert(response);
...
$(function(){
     ...
     $.ajaxSetup({
      ...
      $.ajax({
        type: "GET",
		url: "{{ url('financialsummary/dataRemoveExpensesX')}}/"+id,
		data: {id:id},
        success: function( response ) {
