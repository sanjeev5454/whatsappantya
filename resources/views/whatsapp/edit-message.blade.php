                  ...
                  <div id="formsteps" class="form-group col-lg-12 col-md-12">
                    <div class="form-group row">
                      ...
                      <label>Name of Template</label>
                        <input type="text" class="form-control" name="name_of_template" autocomplete="off" value="{{ $data['name_of_template'] }}" id="name_of_template">
                        ...
                        <div class="form-group row message-text-one">
                          <div class="col-lg-12 msg-tamplate ramdom-tpl">
                            <div class="text-area-box">
                              <textarea class="form-control" placeholder="Message Text" name="message_text_single" autocomplete="off" id="address">{{$data['message_text_single']}}</textarea>
                              @if ($errors->has('message_text_single')) <span class="error">{{ $errors->first('message_text_single') }}</span> @endif <small> Formatting Options ( Italic:_text_, Bold:*text*, Strikethrough:~text~ ) </small>
                              
                            </div>
                            <div class="form-check">
                            <input class="form-check-input" @if($data['random']==1) checked @endif type="checkbox" id="check1" name="random" value="1">
                    ...
                    @php
                    $ext = pathinfo($msg_data['gdrive_name'], PATHINFO_EXTENSION);
                    @endphp
                      <div id="sort_{{ $msg_data['_id'] }}" class="add-row del-temp-{{ $key }}"> 
                        <div class="form-group row data @if($key!=0) del-temp-{{ $key }} @endif">
                          <div class="col-lg-12 msg-tamplate">
                              <input type="hidden" id="hidden_id_image-{{ $key }}" name="hidden_id_image[]" value="{{$hidden_id_image}}">
                              <span class="no-box" style=""></span>
                            <div class="upload-button"> 
                            <img class="img-remove-btn remove-attachment-{{ $key }}" id="{{ $key }}" style="@if($msg_data['gdrive_name']=='') display:none; @endif" src="{{ url('assets/images/icon-delete.png') }}">
                              <label> 
                              <input type="hidden" name="hidden_gdrive_id[]" id="hidden_gdrive_id_{{ $key }}" value="{{ $msg_data['_id'] }}">
                                <input type="hidden" id="hidden_gdrive_{{ $key }}" name="hidden_gdrive[]" value="{{ $msg_data['gdrive_name'] }}">
                                <input type="hidden" id="hidden_original_name_{{ $key }}" name="hidden_original_name[]" value="{{ $msg_data['original_name'] }}">
                              @if($msg_data['gdrive_name']!='') 
                                 <a href="{{ url('public/uploads/thumbnail/') }}/{{ $msg_data['gdrive_name'] }}" target="_blank">
                                 @if($ext=='mp4')
                                 <img id="blah-{{ $key }}" @if($msg_data['gdrive_name']=='') style="display:none;" @endif src="{{ url('public/uploads/mp4.png') }}" width="80" height="80" title="{{ $msg_data['original_name'] }}">
                                 @else
                                 <img id="blah-{{ $key }}" @if($msg_data['gdrive_name']=='') style="display:none;" @endif src="{{ url('public/uploads/thumbnail/') }}/{{ $msg_data['gdrive_name'] }}" width="80" height="80" title="{{ $msg_data['original_name'] }}">
                                 @endif
                                 </a>
                                @endif
                                <input type="file" name="gdrive_name[]" id="gdrive-data-{{$key}}" cus="{{ $key }}"  class="imgInp form-control gdrive-data-{{ $key }} @if($msg_data['gdrive_name']!='') hide-input @endif">
                            ...
                            <div class="loader-box" id="loader-{{$key}}" style="display:none;">
                            <div class="loader">Loading...</div>
                            </div>
                            </div>
                            <div class="text-area-box">
                              <textarea class="form-control" name="message_text[]" autocomplete="off" value="" id="address">{{ $msg_data['caption'] }}</textarea>
                            ...
                            <div class="add-row-btn"><a id="{{$key}}" temp_id="{{ $data['_id'] }}" image_id="{{ $msg_data['_id'] }}" class="add-label btn btn-danger delete-template" href="javascript:void(0);"><i class="fa fa-minus" aria-hidden="true"></i></a></div>
                          </div>
                        </div>
                      </div>
                      @endforeach 
                    ...
                    <div class="form-group row creat-fms-btn">
                      <div class="col-md-12">
                        <button type="submit" class="btn btn-success waves-effect save-temp waves-classic waves-effect waves-classic waves-effect waves-classic"> Update Template</button>
...
$(document).on('click','.delete-template',function(){
     ...
     $.ajax({
        url: "{{ url('whatsapp/imagedatadelete_temp') }}" ,
        type: "POST",
        data: {temp_id:temp_id,image_id:image_id},
