...
@section('title', 'Copy Message Planner')
@section('content')
    ...
    <div class="pane">
      <div class="pane-body">
        <header class="panel-heading">
          <h3 class="panel-title" style="padding:20px 36px 0;">Copy Message Planner</h3>
        </header>
        <div class="new-form"> 
         <h2>Copy Message Planner</h2>
        <div class="row">
          <div class="col-md-10 col-lg-6">
            <div class="card">
              <div class="card-body">
                <form method="POST" action="{{ url('whatsapp/copysendmessage') }}" enctype="multipart/form-data">
			<input type="hidden" value="{{ $data['_id'] }}" name="id" />
               @csrf  
                  ...
                  <div class=" col-lg-12">
					...
					<!--<option @if($data['name_of_sender']==3) selected="selected" @endif value="3"> Select Contact</option>-->
					<!--</select>-->
					<br/>
					<label class="radio-inline">
                    <input type="radio" required @if($data['name_of_sender']==1) checked="checked" @endif name="name_of_sender" value="1"> Add Mobile Number Manually
                    </label>
                    
                    <label class="radio-inline">
                    <input type="radio" required @if($data['name_of_sender']==2) checked="checked" @endif name="name_of_sender" value="2"> Select Group
                    </label>
                    
                    <label class="radio-inline">
                    <input type="radio" required @if($data['name_of_sender']==3) checked="checked" @endif name="name_of_sender" value="3"> Choose Contacts
                    </label>
				</div>
                </div>
            ...
            <label class="radio-inline">
            <input type="radio" required  @if($data['is_recurring']==0) checked @endif name="is_recurring" value="0"> One Time Message
            </label>
            <label class="radio-inline">
            ...
            <div class="recurring_type_box normal_hide recurring_option_hide" style=" @if($data['is_recurring']==1) '' @else display:none; @endif">
              <table class="table table-bordered table-striped table-condensed cf recurring_type inner-table task-table-new">
                ...
                <div class="form-group row send_now" @if($data['is_recurring']==0)  @else style="display:none;" @endif>
                  <div class=" col-lg-12">
                      <div class="center-radio"> 
                      <label class="radio-inline">
                    ...
                    <label class="radio-inline">
                    <input type="radio" @if($data['is_recurring']==0) checked @endif name="send_now" value="2"> Schedule
                    </label>
                     </div>
                     </div>
                 </div>
                ...
                <div class="form-group row">
                  <div class=" col-lg-12">
                    ...
                    <div class="form-group row creat-fms-btn">
                      <div class="col-md-12">
                        <button type="submit" class="btn btn-success waves-effect waves-classic waves-effect waves-classic waves-effect waves-classic msg-send"> Save Message</button>
...
<script>
    $(function(){
        $(document).on('click','.add-contact',function(){
            $('#receiver_mobile_number_contact').val('');
            ...
            //alert(rec_mob_num);
            if(rec_mob_num==''){
            $('#receiver_mobile_number_contact').val(chk);
            $('#con-data').html('<span id="san-'+chk+'">'+chk1+'<span id="'+chk+'" class="remove-btn"></span></span>');
            $('.recurring-task').show();
            }else{
            $('#receiver_mobile_number_contact').val(rec_mob_num+','+chk);
            $('#con-data').html(rec_con_data+'<span id="san-'+chk+'">'+', '+chk1+'<span id="'+chk+'" class="remove-btn"></span></span>');
            $('.recurring-task').show();
            }
