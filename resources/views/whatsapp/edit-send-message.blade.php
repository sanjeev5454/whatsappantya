...
<div class="page creat-fms-box1">
  <div class="page-content container-fluid1">
   ...
   <!--     <h3 class="panel-title">Edit Send Message</h3>-->
   <!--   </header>-->
       <div class="new-form">
        <h2>Edit Send Message</h2>
        <div class="row">
          <div class="col-md-10 col-lg-6">
            <div class="card">
          <div class="card-body">
            <form method="POST" action="{{ url('whatsapp/updatesendmessagemultiple') }}" enctype="multipart/form-data">
			<input type="hidden" value="{{ $data['_id'] }}" name="id" />
			   ...
			   <div id="formsteps" class="form-group col-lg-12 col-md-12">
                <div class="form-group row">
                  <div class=" col-lg-12">
					...
					<option @if($data['name_of_sender']==1) selected="selected" @endif value="1"> Select Mobile Number</option>
					<option @if($data['name_of_sender']==2) selected="selected" @endif value="2"> Select From Group</option>
					</select>
				</div>
                </div>
				
				<div class="form-group group row" style=" @if($data['name_of_sender']==1) display:none; @endif">
                  <div class=" col-lg-12">
                ...
                <div class="form-group input row" style=" @if($data['name_of_sender']==2) display:none; @endif">
                  <div class=" col-lg-12">
                    <label>Receiver mobile no(s)
					</label>
                    <textarea class="form-control" name="receiver_mobile_number" autocomplete="off" value="" id="receiver_mobile_number">{{ $data['receiver_mobile_number'] }}</textarea>
                ...
                <div class="form-group row half-row">
                  <div class=" col-lg-6">
                    <label>Date</label>
                    <input type="text" class="form-control" required name="message_date" autocomplete="off" value="{{ $data['message_date'] }}" id="message_date">
                  ...
                  <div class=" col-lg-6">
                    ...
                    @endphp
                    @endif
                    <option @if($data['message_time']==$i) selected @endif value="{{ $i }}">{{ $i.':00' }}</option>
                    @endfor
                    </select>
                  </div>
                </div>
				...
				<div id="append-text" class="add-box"> @foreach($message_data as $key=>$msg_data)
                      <div class="add-row @if($key!=0) del-temp-{{ $key }} @endif"> 
                          ...
                          <div class="col-lg-12 msg-tamplate">
                            ...
                            <img class="img-remove-btn remove-attachment-{{ $key }}" id="{{ $key }}" style="@if($msg_data['gdrive_name']=='') display:none; @endif" src="{{ url('assets/images/icon-delete.png') }}">
                              ...
                              @if($msg_data['gdrive_name']!='') 
                                 ...
                                 <img id="blah-{{ $key }}" @if($msg_data['gdrive_name']=='') style="display:none;" @endif src="{{ url('public/uploads/thumb/') }}/{{ $msg_data['gdrive_name'] }}" title="{{ $msg_data['original_name'] }}"></a>
                                ...
                                @endif
                                <input type="file" name="gdrive_name[]" cus="{{ $key }}"  class="imgInp form-control gdrive-data-{{ $key }} @if($msg_data['gdrive_name']!='') hide-input @endif">
                            ...
                            <div class="text-area-box">
                              <textarea class="form-control" name="message_text[]" autocomplete="off" value="" id="address">{{ $msg_data['message_text'] }}</textarea>
                            ...
                            <div class="add-row-btn"><a id="{{$key}}" class="add-label btn @if($key!=0) btn-danger @else btn-success @endif @if($key!=0) delete-template @else add-template @endif " href="javascript:void(0);"><i class="fa @if($key!=0) fa-minus @else fa-plus @endif " aria-hidden="true"></i></a></div>
                          </div>
                        </div>
                      </div>
                      @endforeach </div>
              ...
              <div class="form-group row creat-fms-btn">
                <div class="col-md-12">
                  <button type="submit" class="btn btn-success waves-effect waves-classic waves-effect waves-classic waves-effect waves-classic"> Update Send Message</button>
				  <!--<a href="{{ url('whatsapp/save-message-listing') }}" title="Back"><button type="button" class="btn btn-danger waves-effect waves-classic waves-effect waves-classic waves-effect waves-classic"> Back</button></a>-->
                  </div>
              </div>
			  </div>
            </form>
