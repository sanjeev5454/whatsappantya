        ...
        <div class="tab-content tab-table-fix" id="myTabContent">
          <div class="table-layout-fixed message-planner-box purchase-order-listing-box" id="billed" role="tabpanel" aria-labelledby="billed-tab">
            <table class="table responsive-table dataTable" cellspacing="0" id="exampleAddRow1">
              ...
              @php
              $qty_data = quantityUpdateDetails($data['update_id']);
              //echo $data['invoice_number'];
              //pr($qty_data);
              //echo '-';
              //echo $data['update_id'];
              //echo purchaseOrderDetails($qty_data['order_number'])->contact.'-'.$data['invoice_number'];
              @endphp
              @if(purchaseOrderDetails($qty_data['order_number'])->contact!='')
              <tr class="gradeA gr @if(getApproveData($data['invoice_number'])['status']==1) green-line @else red-line @endif line-{{ $data['invoice_number'] }} s-{{ purchaseOrderDetails($qty_data['order_number'])->contact }}">
                <td data-table="Vendor" class="show-hide-tab"><span>{{ purchaseOrderDetails($qty_data['order_number'])->contact }}</span></td>
                <td data-table="Invoice Number">{{ $data['invoice_number'] }}</td>
                <td data-table="Invoice Date">{{ $data['invoice_date'] }}</td>
                <td data-table="Status" id="sta-{{ $data['invoice_number'] }}"> @if(getApproveData($data['invoice_number'])['status']==1) Approved by {{ userDetails(getApproveData($data['invoice_number'])['user_id'])->name }} on {{ date('jS M Y',getApproveData($data['invoice_number'])['created_time'])}} @else Not approved yet @endif</td>
              </tr>
              
              <tr class="display-none has-table gr1">
                <td colspan="3" align="center">
              <table class="table responsive-table dataTable" cellspacing="0" cellpadding="0">
                <tr>
                <thead>
                  <th class="items-th Items-box">Items</th>
                  <th class="center q-ordered-th q-ordered-box">Q.Ordered</th>
                  <th class="center unit-th unit-box">Unit</th>
                  <th class="center pric-th price-box">Price</th>
                  <th class="center q-received-th q-received-box">Q.Received</th>
                  <th class="center price-th price-box">Price</th>
                  <th class="center total-th total-box">Total</th>
                  </thead>
                </tr>
              @php
              $subtotal = 0;
              ...
              @php
              $qty_data_item = quantityUpdateDetails($update_records['update_id']);
              @endphp
              @if($qty_data_item['item']!='')
              @php
              $subtotal += $update_records['received_price']*$update_records['qty_received'];
              $subtotal_gst += ($update_records['received_price']*$update_records['qty_received'])*ItemDetailsgst($qty_data_item['item'])/100;
              @endphp
              <tr class="gradeA">
                <td data-table="Items"><span>{{ ItemDetails($qty_data_item['item']) }}</span></td>
                <td class="center" data-table="Qty">{{ $qty_data_item['quantity'] }}</td>
                <td class="center" data-table="Unit">{{ $qty_data_item['unit'] }}</td> 
                <td class="center" data-table="Price">{{ $qty_data_item['price'] }}</td>
                <td class="center" data-table="Received Qty">{{ $update_records['qty_received'] }}</td>
                <td class="center" data-table="Price">{{ $update_records['received_price'] }}</td>
                <td class="center" data-table="Total">{{ $update_records['qty_received']*$update_records['received_price'] }}</td>
              </tr>
              @endif
              @endif
              @endforeach
              ...
              <tr>
                  <td colspan="6" align="right">Total (Including GST) : </td>
                ...
                <td id="san-{{ $data['invoice_number'] }}">
                    ...
                    @if(Auth::user()->role_id=="4" || Auth::user()->user_role=="1")
                    ...
                    @endif
                    @else
                    Approved by <br/>{{ userDetails(getApproveData($data['invoice_number'])['user_id'])->name }} <br/>on {{ date('jS M Y',getApproveData($data['invoice_number'])['created_time'])}}
                    @endif
