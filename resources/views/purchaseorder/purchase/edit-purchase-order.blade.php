        ...
        <form class="" action="{{ url('purchaseorder/new-purchase-order-update/'.$purchase_data->_id) }}" method="post" autocomplete="on" onsubmit="return validate();">
              ...
              <div class="purchaseorder-box">
                <div class="search-form">
                  <div class="search-form-box">
                    ...
                    <div class="date-form">
                      <label for="order-number instructed_by">Instructed By</label>
                      <select name="instructed_by" class="form-control" required>
                          <option value="">-- Select --</option>
                          <option @if($purchase_data->instructed_by==InstructedByAdmin()->_id) selected @endif value="{{ InstructedByAdmin()->_id }}">{{ InstructedByAdmin()->name }}</option>
                          @foreach(InstructedBy() as $ins)
                          <option @if($purchase_data->instructed_by==$ins->_id) selected @endif value="{{$ins->_id}}">{{ $ins->name }}</option>
                          @endforeach
                      </select>
                    </div>
                  </div>
                ...
                <div class="table-wrap">
                  <table class="editable-table table table-striped responsive-table dataTable purchase-order-table" id="editableTable1">
                    <thead>
                      <tr>
                        <th width="170" class="center">Item</th>
					...
					@endforeach
                      ...
                      <td data-table="Item SKU" id="data-row-item-sku-{{ $i }}"> <span class="blankspan">{{ ItemSKU($purchase_data->row[$i]['item']) }}</span> </td>
                      ...
                      <td data-table="Quantity"  class="center"><input type="text" required min="1" autocomplete="off" class="form-control width-full quantity" cus="{{$i}}" value="{{ $purchase_data->row[$i]['quantity'] }}" rel="quantity" id="tmp_id_qty_{{$i}}" name="row[{{$i}}][quantity]" /></td>
                      <td class="center" data-table="Size"><select class="form-control width-full" name="row[{{$i}}][size]"><option value="">-- Size --</option><option @if($purchase_data->row[$i]['size']==30) selected @endif value="30">30</option><option @if($purchase_data->row[$i]['size']==31) selected @endif value="31">31</option><option @if($purchase_data->row[$i]['size']==32) selected @endif value="32">32</option><option @if($purchase_data->row[$i]['size']==33) selected @endif value="33">33</option><option @if($purchase_data->row[$i]['size']==34) selected @endif value="34">34</option><option @if($purchase_data->row[$i]['size']==35) selected @endif value="35">35</option><option @if($purchase_data->row[$i]['size']==36) selected @endif value="36">36</option><option @if($purchase_data->row[$i]['size']==37) selected @endif value="37">37</option><option @if($purchase_data->row[$i]['size']==38) selected @endif value="38">38</option><option @if($purchase_data->row[$i]['size']==39) selected @endif value="39">39</option><option @if($purchase_data->row[$i]['size']==40) selected @endif value="40">40</option><option @if($purchase_data->row[$i]['size']==41) selected @endif value="41">41</option><option @if($purchase_data->row[$i]['size']==42) selected @endif value="42">42</option><option @if($purchase_data->row[$i]['size']==43) selected @endif value="43">43</option><option @if($purchase_data->row[$i]['size']==44) selected @endif value="44">44</option></select></td>
                      <td data-table="Unit" class="center unit-td"><input type="text" class="form-control width-full" value="{{ $purchase_data->row[$i]['unit'] }}" rel="unit" id="tmp_id_unit_{{$i}}" name="row[{{$i}}][unit]" readonly>
                        </td>
                      <td data-table="Price" class="center"><input required type="text" autocomplete="off" min="1" class="form-control width-full price" value="{{ $purchase_data->row[$i]['price'] }}" cus="{{$i}}" rel="price" id="tmp_id_price_{{$i}}" name="row[{{$i}}][price]" /></td>
                      <td data-table="Disc %" class="center disc-td"><input type="text" min="1" autocomplete="off" maxlength="2" value="{{ $purchase_data->row[$i]['disc'] }}" class="form-control width-full disc" cus="{{$i}}" rel="disc" id="tmp_id_disc_{{$i}}" name="row[{{$i}}][disc]" /></td>
                      <td data-table="Amount INR" id="data-row-amount-{{ $i }}" class="amount center" style="font-weight:bold;" align="right"><span class="blankspan">{{ $purchase_data->row[$i]['amount'] }}</span> </td>
                      <td data-table="GST %" class="center gst-td"><input type="text" min="1" readonly="" autocomplete="off" maxlength="3" value="{{ $purchase_data->row[$i]['gst'] }}" class="form-control width-full gst" rel="gst" cus="{{$i}}" id="tmp_id_gst_{{$i}}" name="row[{{$i}}][gst]" />
                ...
                <div class="add-row">
                  ...
                  <div class="row address-wrap">
                    ...
                    <div class="col-md-3 address-bydefault">
                      ...
                      <input type="hidden" id="telephone" name="address[telephone]" value="{{ $purchase_data->address['telephone'] }}" class="form-control">
                    </div>
                    ...
                    <div class="col-md-3">
                      ...
                      <label>Send Copy Of The PO To</label>
                        ...
                        <div class="checkbox">
                        <label><input type="checkbox" @if(in_array($staffs->_id,$copy_of_staffs)) checked @endif  name="copy_of_staffs[]" value="{{$staffs->_id}}"> {{$staffs->name}}</label>
                        </div>
                        @endforeach
                    </div>
                    </div>
		  ...
		  <a href="javascript:void(0)" class="btn btn-primary" style="display:none;">Save</a> <a href="javascript:void(0)" class="btn btn-secondary cancel-btn">Cancel</a>
           ...
           <!-- Modal -->
            ...
            <div class="radio">
            <label><input type="radio" @if($purchase_data->code_vendor==1) checked @endif name="code_vendor" value="1">  SHOW VENDOR DETAILS to Staff (Full name, Address and Contact Details).</label>
            </div>
            <div class="radio">
            <label><input type="radio" @if($purchase_data->code_vendor==0) checked @endif name="code_vendor" value="0">  HIDE VENDOR DETAILS from Staff (Only Vendor Code will be displayed).</label>
            </div>
            </div>
