...
.purchaseorder-box .search-form .search-form-box .date-form{ width:163px; }
  ...
  <div class="page-content container-fluid">
    <div class="row justify-content-md-center">
      <div class="col-md-12">
        ...
        <?php } ?>
        <form class="" action="{{ url('purchaseorder/new-purchase-order-update/'.$purchase_data->_id) }}" method="post" autocomplete="on" onsubmit="return validate();">
          @csrf
          <div class="card">
            <div class="card-body">
              <div class="purchaseorder-box">
                <div class="search-form">
                  <div class="search-form-box">
                    <div class="col-md-12 approved-box">
                     <div class="message_receive_by">
                         @if($purchase_data->status==0) Draft @elseif($purchase_data->status==1) <span style="color:green;">Approved</span> @elseif($purchase_data->status==2) <span style="color:green;">Awaiting Approval</span> @elseif($purchase_data->status==3) <span style="color:green;">Billed</span> @endif
                     @if($purchase_data->message_receive_type!='')
                     <span class="message_receive"><strong>Sent by</strong> {{$purchase_data->message_receive_type}} {{$purchase_data->message_receive}}</span>
                     @endif
                     </div>
                    <span class="approved-box-btn">
                    <div class="add-left">
                        <div class="dropdown two-button show">
                          ...
                          <div class="dropdown-menu" x-placement="bottom-start" style="position: absolute; will-change: transform; top: 0px; left: 0px; transform: translate3d(78px, 34px, 0px);"> <a class="dropdown-item add-new-line" cus="5" href="{{ url('purchaseorder/edit-purchase-order/'.$purchase_data->_id) }}">Edit</a> 
                          @if(Auth::user()->user_role==1)
                          <a class="dropdown-item add-new-line" href="{{ url('purchaseorder/purchaseDelete/'.$purchase_data->_id) }}" onclick="return confirm('Are you sure to delete the row?');">Delete</a> 
                          @endif
                          </div>
                        </div>
                      </div><a data-toggle="modal" data-target="#myModal" class="btn btn-default waves-effect waves-classic">Send Again</a><span class="download-btn download-dropdown">
                    <a href="javascript:void(0)" class="btn"><i class="icon md-download" aria-hidden="true"></i> Download</a>
                    <ul>
                        @if(Auth::user()->user_role==1)
                        <!--<li><a href="{{ url('purchaseorder/vendor-desktop-generate-pdf/'.$purchase_data->_id) }}"><i class="icon md-download" aria-hidden="true"></i>Vendor Desktop Download</a></li>-->
                        <li><a href="{{ url('purchaseorder/vendor-mobile-generate-pdf/'.$purchase_data->_id) }}"><i class="icon md-download" aria-hidden="true"></i>Vendor Download</a></li>
                        <li><a href="{{ url('purchaseorder/staff-mobile-generate-pdf/'.$purchase_data->_id) }}"><i class="icon md-download" aria-hidden="true"></i>Staff Download</a></li>
                        <!--<li><a href="{{ url('purchaseorder/staff-desktop-generate-pdf/'.$purchase_data->_id) }}"><i class="icon md-download" aria-hidden="true"></i>Staff Desktop Download</a></li>-->
                        @else
                        <li><a href="{{ url('purchaseorder/staff-mobile-generate-pdf/'.$purchase_data->_id) }}"><i class="icon md-download" aria-hidden="true"></i>Staff Download</a></li>
                        <!--<li><a href="{{ url('purchaseorder/staff-desktop-generate-pdf/'.$purchase_data->_id) }}"><i class="icon md-download" aria-hidden="true"></i>Staff Desktop Download</a></li>-->
                        @endif
                      ...
                      <!--<a href="{{ url('purchaseorder/generate-pdf/'.$purchase_data->_id) }}" class="btn btn-default waves-effect waves-classic">Download</a> -->
                      </span>
                    ...
                    <div class="contact-form bs-example">
                      ...
                      <span style="color:#000000;">{{ $purchase_data->contact }}</span> </div>
                    <div class="date-form">
                      <label for="address">Address</label>
                      <span style="color:#000000;">{{ vendorDetails($purchase_data->contact)->address }}</span> </div>
                    <div class="date-form">
                      <label for="email">Email</label>
                      <span style="color:#000000;">{{ vendorDetails($purchase_data->contact)->email }}</span> </div>
                    <div class="date-form">
                      <label for="date">Date</label>
                      <span style="color:#000000;">{{ $purchase_data->date }}</span> </div>
                    <div class="date-form">
                      <label for="delivery-date">Delivery Date</label>
                      <span style="color:#000000;">{{ $purchase_data->delivery_date }}</span> </div>
                    <div class="date-form">
                      <label for="order-number">Order number</label>
                      <span style="color:#000000;">{{ 'PO-'.$purchase_data->order_number }}</span> </div>
                  </div>
                  <hr />
                  ...
                  <table class="editable-table table table-striped responsive-table dataTable purchase-order-table" id="editableTable1">
                    <thead>
                      <tr>
                        ...
                        <th width="90">Disc %</th>
                        <th width="110" class="lga">Amount INR</th>
                        <th width="90">GST %</th>
                      </tr>
                    </thead>
                    <tbody id="results">
                    ...
                    <tr id="data-row-{{ $i }}">
                      <td data-table="Item"> {{ ItemDetails($purchase_data->row[$i]['item']) }}  @if(ItemDetailsUpdateQty($purchase_data->_id)!='')<span style="color:#063; font-size:9px; float:right;"> <strong>Received [{{ ItemDetailsUpdateQty($purchase_data->_id) }}]</strong><br />
                        <strong>{{ ItemDetailsUpdateDate($purchase_data->_id) }}</strong></span> @endif </td>
                      <td data-table="Item SKU" id="data-row-item-sku-{{ $i }}"> {{ ItemSKU($purchase_data->row[$i]['item']) }} </td>
                      <td data-table="Quantity"> {{ $purchase_data->row[$i]['quantity'] }} </td>
                      <td data-table="Size"> {{ $purchase_data->row[$i]['size'] }} </td>
                      <td data-table="Unit"> {{ $purchase_data->row[$i]['unit'] }} </td>
                      <td data-table="Price"> {{ $purchase_data->row[$i]['price'] }} </td>
                      <td data-table="Disc %"> {{ $purchase_data->row[$i]['disc'] }} </td>
                      <td data-table="Amount INR" id="data-row-amount-{{ $i }}" class="amount" style="font-weight:bold;" align="right"> {{ $purchase_data->row[$i]['amount'] }} </td>
                      <td data-table="GST %"> {{ $purchase_data->row[$i]['gst'] }} </td>
                    </tr>
                    @endfor
                      </tbody>
                ...
                <div class="add-row">
                  ...
                  <div class="row address-wrap">
                    ...
                    <div class="col-md-3 address-select" style="display:none;">
                      <label>Address</label>
                      {{$purchase_data->address['new_delivery_address']}} </div>
                    <div class="col-md-3">
                      <div class="form-box">
                        <label>Kind Attention</label>
                        {{$purchase_data->address['attention']}} </div>
                    </div>
                    <div class="col-md-6">
                      <label>Notes</label>
                      {{stripslashes($purchase_data->address['instruction'])}} </div>
                  </div>
                </div>                
		  ...
		  {{stripslashes($purchase_data->history_note)}} 
                ...
                <div class="save-row"> @if($purchase_data->status==0)
                  ...
                  <button type="submit" name="awaiting" value="save" class="btn btn-primary">Save & submit for approval</button> @endif
                  <div class="save-right float-right"> <a href="{{ url('purchaseorder/purchase-order-listing') }}"  class="btn btn-secondary">Cancel</a> </div>
                </div>
              </div>
            </div>
          </div>
...
<div class="modal fade" id="myModal" role="dialog">
  <div class="modal-dialog">
    <form class="" action="{{ url('purchaseorder/purchase-order-send/'.$purchase_data->_id) }}" method="post" autocomplete="on">
      @csrf 
      ...
      <div class="modal-content">
        ...
        <div class="modal-body">
          ...
          <hr />
          <strong>To</strong>:
          <input type="text" class="form-control width-full" value="{{ vendorDetails($purchase_data->contact)->email }}" id="to_send_email" name="to_send_email">
          <div style="margin-top:20px;"> <strong>Message</strong>:
            <input type="text" class="form-control width-full" value="Purchase Order PO-{{ $purchase_data->order_number }} from {{ userDetails(Auth::id())->name }} for {{ $purchase_data->contact }}" id="to_send_subject" name="to_send_subject">
          </div>
          <div style="margin-top:20px;">
            <textarea class="form-control width-full" wrap="hard" style="height:200px;white-space: pre-line;text-align: left;-moz-text-align-last: left;text-align-last: left;" rows="4" cols="20" name="to_send_message_body">Dear {{ $purchase_data->contact }},

Attached please find purchase order #PO-{{ $purchase_data->order_number }} for INR {{ $purchase_data->total }}.

If you have any questions, please do not hesitate to call me back.
...
(Partner, ANTYA)
          ...
          <div class="checkbox-custom checkbox checkbox-primary checkbox-sm">
            <input type="checkbox" @if(@in_array('sms',vendorDetails($purchase_data->
            contact)->notification)) checked @endif  class="checkBoxClass" id="inputCheckboxsms" value="1" name="smssend">
            <label for="inputCheckboxsms">SMS (Text Message)</label>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <input type="checkbox" @if(@in_array('whatsup',vendorDetails($purchase_data->
            contact)->notification)) checked @endif class="checkBoxClass" id="inputCheckboxwhatsup" value="1" name="whatsappsend">
            <label for="inputCheckboxwhatsup">Whatsapp</label>
            &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
            <input type="checkbox" class="checkBoxClass" @if(@in_array('mail',vendorDetails($purchase_data->
            contact)->notification)) checked @endif id="inputCheckboxmail" value="1" name="mailsend">
