  ...
  <div class="new-form">
    ...
    <div class="row">
      <div class="col-md-6 col-lg-6">
        ...
        <div class="card">
          <div class="card-body">
            <form method="POST" action="{{ url('purchaseorder/itemDataUpdate/'.$item_data->id) }}" enctype="multipart/form-data" onsubmit="return vendorDataPostValidate();">
              @csrf
              <div id="formsteps" class="form-group col-lg-12 col-md-12">
                <div class="form-group row">
                  <div class=" col-lg-12">
                    ...
                    @if ($errors->has('item_name')) <span class="error">{{ $errors->first('item_name') }}</span> @endif </div>
                </div>
                <div class="form-group row">
                  <div class=" col-lg-12">
                    <label>Item SKU</label>
                    <input type="text" class="form-control" name="item_sku" autocomplete="off" value="{{ $item_data->item_sku }}" id="item_sku">
                    @if ($errors->has('item_sku')) <span class="error">{{ $errors->first('item_sku') }}</span> @endif </div>
                </div>
                <div class="form-group row">
                  <div class=" col-lg-12">
                ...
                <div class="form-group row">
                  <div class=" col-lg-12">
                    ...
                    @if ($errors->has('unit')) <span class="error">{{ $errors->first('unit') }}</span> @endif </div>
                </div>
                <div class="form-group row other-unit" @if($item_data->unit=='Other') {{ '' }} @else style="display:none;" @endif >
                  <div class=" col-lg-12">
                    ...
                    <small class="text-muted">Add other unit separated by comma (,).</small> @if ($errors->has('other_unit')) <br/>
                    <span class="error">{{ $errors->first('other_unit') }}</span> @endif </div>
                </div>
                <div class="form-group row">
                  <div class=" col-lg-12">
                    ...
                    @if ($errors->has('price')) <span class="error">{{ $errors->first('price') }}</span> @endif </div>
                </div>
                <div class="form-group row">
                  <div class=" col-lg-12">
                    ...
                    <option value="">-- Select Vendor --</option>
                    @foreach($vendors as $all_vendor)
                    <option @if($item_data->vendor_id==$all_vendor->id) selected @endif value="{{ $all_vendor->id }}">{{ $all_vendor->vendor_code }}</option>
                    @endforeach
                    </select>
					@endif
                    @endif
                    @if ($errors->has('vendor_id')) <span class="error">{{ $errors->first('vendor_id') }}</span> @endif </div>
                </div>
                <div class="form-group row">
                  <div class=" col-lg-12">
                    <label>GST (in %)</label>
                    ...
                    @if ($errors->has('gst')) <span class="error">{{ $errors->first('gst') }}</span> @endif </div>
                </div>
                <div class="form-group row creat-fms-btn">
                  <div class="col-md-12">
                    ...
                    <!--<a href="{{ url('purchaseorder/item-listing') }}" title="Back">
                    <button type="button" class="btn btn-danger waves-effect waves-classic waves-effect waves-classic"> Back</button>
                    </a>--> </div>
                </div>
              </div>
            </form>
