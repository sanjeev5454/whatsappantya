...
<div class="page">
  ...
  <div class="new-form">
	...
	<?php } ?>
        <div class="card">
          <div class="card-body">
            <form method="POST" action="{{ url('purchaseorder/itemDataPost') }}" enctype="multipart/form-data" onsubmit="return itemDataPostValidate();">
              @csrf
              <div id="formsteps" class="form-group col-lg-12 col-md-12">
				...
				<div class="form-group row">
                  <div class=" col-lg-12">
					...
					<option value="">-- Select Vendor --</option>
					@foreach($vendors as $all_vendor)
					<option @if (old('vendor_id') == $all_vendor->id) {{ 'selected' }} @endif value="{{ $all_vendor->id }}">{{ $all_vendor->vendor_code }}</option>
					@endforeach
					</select>
					@endif
					@endif
              ...
              <div class="form-group row creat-fms-btn">
                <div class="col-md-12">
				  ...
				  <!--<a href="{{ url('purchaseorder/item-listing') }}" title="Back"><button type="button" class="btn btn-danger waves-effect waves-classic waves-effect waves-classic"> Back</button></a>-->
                  </div>
              </div>
			  </div>
            </form>
