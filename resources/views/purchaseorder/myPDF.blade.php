...
<table width="100%" border="0" cellpadding="3" cellspacing="0">
  ...
  <tr>
    <td><table  width="100%" border="0" cellpadding="5" cellspacing="0" class="border-table">
  <tr>
    <th style="background-color: #e5e5e5;" width="35px">Item</th>
    <td width="140px" style="text-align:left">{{ ItemDetails($purchase_data->row[$i]['item']) }}</td>
    <th width="35px" style="background-color: #e5e5e5;">Size</th>
    <th width="35px" style="background-color: #e5e5e5;">Unit</th>
    <th style="background-color: #e5e5e5;"  width="50px">Price</th>
    <th style="background-color: #e5e5e5;" width="30px">Qty</th>
    <th width="35px" style="background-color: #e5e5e5;">Disc.%</th>   
    <td rowspan="3" valign="center" width="60px"><br>
    <br>
      {{ $purchase_data->row[$i]['amount'] }}</td>
  </tr>
  <tr>
    <th width="35px" style="background-color: #e5e5e5;">SKU</th>
    <td width="140px" style="text-align:left">{{ ItemSKU($purchase_data->row[$i]['item']) }}</td>
    <td>{{ $purchase_data->row[$i]['size'] }}</td>
    <td>{{ $purchase_data->row[$i]['unit'] }}</td>
    <td>{{ number_format($purchase_data->row[$i]['price'],2) }}</td>
    <td width="30px">{{ $purchase_data->row[$i]['quantity'] }}</td>
    <td width="35px">{{ $purchase_data->row[$i]['disc'] }}</td>
  </tr>
</table>
</td>
  </tr>
