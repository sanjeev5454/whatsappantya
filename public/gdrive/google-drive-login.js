...
var CLIENT_ID = '552966328077-2902qqo49u6tllvkbs5mnuo9gblrlr53.apps.googleusercontent.com';
var API_KEY = 'AIzaSyDuB9cQHmjznS8KCsKlDbCHZb4xNTRfqtA';
var FOLDER_NAME = "";
var FOLDER_ID = "root";
    ...
    if (DRIVE_FILES.length > 0) {
        for (var i = 0; i < DRIVE_FILES.length; i++) {
			...
			if (DRIVE_FILES[i].fileType != "file") {
				fText += "<div class='folder-icon' data-level='" + DRIVE_FILES[i].level + "' data-parent='" + DRIVE_FILES[i].parentID + "' data-size='" + DRIVE_FILES[i].fileSize + "' data-id='" + DRIVE_FILES[i].id + "' title='" + textTitle + "' data-name='" + DRIVE_FILES[i].title + "' data-has-permission='" +DRIVE_FILES[i].hasPermission + "'><div class='image-preview'><img src='public/gdrive/images/folder.png'/></div></div>";
			} else {
				if (DRIVE_FILES[i].thumbnailLink) {
					...
					if(DRIVE_FILES[i].fileExtension=='PDF'){
						var ext = 'pdf';
					}else{
						var ext = DRIVE_FILES[i].fileExtension;
					}
					fText += "<div class='file-icon'><div class='image-preview'><img src='public/gdrive/images/" + ext + "-icon.png" + "'/></div></div>";
				}
			}
			...
			fText += "<div class='button-box'>";
				...
				if (DRIVE_FILES[i].fileType != "folder") {
				fText += "<div class='button-select' title='Select' data-id='" + DRIVE_FILES[i].id + "'><input name='chk' cus='ch-"+[i]+"' class='chk' type='checkbox' id='"+DRIVE_FILES[i].id+"'></div>";
				fText += "<div class='choose-file' data-id='" + DRIVE_FILES[i].id + "'><input style='display:none;' data-dismiss='modal' name='"+DRIVE_FILES[i].title+"' class='btn-save btn btn-sm btn-success sel-data-ch-"+[i]+"' value='Select' type='button' id='"+DRIVE_FILES[i].id+"'></div>";
				}
...
function showLoading() {
    ...
    $("#drive-box-loading").html("<div id='loading-wrapper'><div id='loading'><img src='public/gdrive/images/loading-bubble.gif'></div></div>");
}
